...
![過去のニュース](docs/screenshots/archive.png)

- **できること**：直近1週間の記事のみをダッシュボードに表示し、それ以前の記事は`/archive`ページに分離して表示します。
- **価値**：記事が蓄積してもダッシュボードの見通しが保たれ、優先度・カテゴリでの絞り込みやブックマークは引き続き利用できます。
